...
data = mat73.loadmat(r"/home/users/jbi/Bachelorarbeit/Codebase/Netze/MIONet/annular_P_v_IC_nl_data/annular_P_v_IC_vvar_nl_40k_16k_20x.mat")
...
def train_deeponet(config,model_id):
    # Modellverzeichnis
    model_dir = f"/home/users/jbi/Bachelorarbeit/Codebase/Netze/MIONet/annular_P_v_IC_nl_models/Parameterstudien_vvar/P_v_IC_20x_V2/{model_id}"
    ...
    n_points_rd = n_points//4
    n_points_v = n_points//4

    v_data = config["v_data"]

    if v_data == "True":
        xyz_t_train = np.concatenate((xyz_t_bias_train[:n_points], xyz_t_rd_train[:n_points_rd], xyz_t_v_train[:n_points_v]), axis=0)
        xyz_t_test = np.concatenate((xyz_t_bias_test[:n_points], xyz_t_rd_test[:n_points_rd], xyz_t_v_test[:n_points_v]), axis=0)
        
        u_train = np.concatenate((u_bias_train[:,:n_points], u_rd_train[:,:n_points_rd], u_v_train[:,:n_points_v]), axis=1)
        u_test = np.concatenate((u_bias_test[:,:n_points], u_rd_test[:,:n_points_rd], u_v_test[:,:n_points_v]), axis=1)
    elif v_data == "False":
        xyz_t_train = np.concatenate((xyz_t_bias_train[:n_points], xyz_t_rd_train[:n_points_rd]), axis=0)
        xyz_t_test = np.concatenate((xyz_t_bias_test[:n_points], xyz_t_rd_test[:n_points_rd]), axis=0)
        
        u_train = np.concatenate((u_bias_train[:,:n_points], u_rd_train[:,:n_points_rd]), axis=1)
        u_test = np.concatenate((u_bias_test[:,:n_points], u_rd_test[:,:n_points_rd]), axis=1)
...
for neurons in (200,300,100):
    for n_points,n_train in((8000,4000),(16000,4000),(8000,8000)):
            layers = 5 #for layers in (4,6):
            act_IC = "relu"#for act_IC in ("relu","swish"):
            act_branch = "relu"#    for act_branch in ("relu","swish"):
            #n_train = 8000
            #n_points = 4000
            v_data = "True"#for v_data in ("True", "False"):
            layers = 5 #for layers in (4,6):
...
for i, config in enumerate(configs):
    ...
    print(results[0])
    keys = results[0].keys()
    with open('/home/users/jbi/Bachelorarbeit/Codebase/Netze/MIONet/annular_P_v_IC_nl_models/Parameterstudien_vvar/P_v_IC_20x_V2/train_results.csv', 'w', newline='') as output_file:
